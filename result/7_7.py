...
def check(poly, L_matrix, bit_size):
    poly_list = poly
    ...
    for p in poly_list:
        poly_obj = P(p)
        mat = sum(c * (X**i) if i > 0 else c * I for i, c in enumerate(poly_obj.list()))
...
if __name__ == "__main__":
    ...
    print("Generating 7x7 matrix from path...")
    m = gen_matrix_from_path(path)

    print("Calculating minors (this may take a moment for 7x7)...")
    poly_list = get_minors(m)

    assert poly_list != "Not MDS", "Matrix is not natively MDS"
